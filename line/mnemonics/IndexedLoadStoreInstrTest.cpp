...
TEST_CASE("Test IndexedLoadStoreInstr parse") {
    std::vector<std::tuple<std::string, std::vector<std::string>,
        ...
        good = {
            ...
            {"ST",
             {"+S,$off"},
             {"ST", IncDecRegister(Register::S, false, false),
              Symbol(Name("off"))}},
            {"ST",
             {"X-,34"},
             {"ST", IncDecRegister(Register::X, true, true),
              Symbol(Number(0x34))}},
        };

    for (size_t i = 0; i < good.size(); i++) {
        REQUIRE(*IndexedLoadStoreInstr::parseOp(std::get<0>(good[i]),
                                                std::get<1>(good[i])) ==
                std::get<2>(good[i]));
    }
    ...
    for (size_t i = 0; i < bad.size(); i++) {
        REQUIRE_THROWS_AS(IndexedLoadStoreInstr::parseOp(std::get<0>(bad[i]),
                                                         std::get<1>(bad[i])),
                          ParseExn);
    }
}
...
TEST_CASE("Test IndexedLoadStoreInstr encode") {
    ...
    std::vector<std::pair<IndexedLoadStoreInstr, uint16_t>> good = {
        // Loads
        // X
        {{"LD", IncDecRegister(Register::X), Symbol(Name("const1"))}, 0x97C1},
        // Pre-increment X
        {{"LD", IncDecRegister(Register::X, false, false),
          Symbol(Number(0x20))},
         0x8620},
        // S
        {{"LD", IncDecRegister(Register::S), Symbol(Name("const2"))}, 0x9322},
        // Pre-decrement S
        ...
        // Stores
        // X
        {{"ST", IncDecRegister(Register::X), Symbol(Number(0x90))}, 0xB790},
        // Post-decrement X
