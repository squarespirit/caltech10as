...
TEST_CASE("Test ConstOp apply") {
    ...
    // Name already exists
    REQUIRE_THROWS_AS(ConstOp(Name("const2"), Number(0x30)).apply(c), NameExn);
}

TEST_CASE("Test ConstOp parse") {
    std::vector<std::tuple<std::string, std::vector<std::string>, Name, Number>>
        good = {
            {".const", {"_myvar", "123"}, Name("_myvar"), Number("123")},
        };
    for (size_t i = 0; i < good.size(); i++) {
        REQUIRE(*ConstOp::parseOp(std::get<0>(good[i]), std::get<1>(good[i])) ==
    ...
    for (size_t i = 0; i < bad.size(); i++) {
        REQUIRE_THROWS_AS(ConstOp::parseOp(bad[i].first, bad[i].second),
                          ParseExn);
    }
}